SELECT 'step4ab-wcte-insert-select-from-held' AS step;
SET ROLE rig_app;
...
SELECT count(*) AS appts_before FROM appointments;
WITH held AS (
...
SELECT h.tenant_id, '99999999-9999-4999-8999-999999999999', h.hospital_id, h.doctor_id,
  ...
  FROM held h;
SELECT count(*) AS appts_after FROM appointments;
SELECT idempotency_key FROM appointments ORDER BY created_at;
RESET ROLE;
