...
-- Supabase migration 01 (baseline DDL, PostgreSQL 16 / Supabase Postgres)
-- Source: database/schema.sql  |  Supabase notes: database/README.md section 11
--
-- Conventions:
...
-- Supabase adaptation: tenant identity comes from the request JWT via
-- auth.jwt() (app_metadata.tenant_id) instead of a session GUC. PostgREST
-- enforces these policies for anon/authenticated clients automatically;
-- service_role / superusers (used by the voice pipeline) bypass them.
...
DECLARE
  tid text;
BEGIN
  -- NOTE: (auth.jwt() -> 'app_metadata') ->> 'tenant_id'.
  -- Chained ->> is wrong (->> yields text, which has no ->> operator).
  tid := (auth.jwt() -> 'app_metadata') ->> 'tenant_id';
  RETURN NULLIF(tid, '')::uuid;
EXCEPTION WHEN OTHERS THEN
  -- No/invalid JWT (anon without claims, malformed uuid): fail closed -> deny.
  RETURN NULL;
END;
$$;
